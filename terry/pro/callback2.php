...
$conn=new PDO("mysql:host=localhost;dbname=otrsphp","root","");
$conn->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);

$stmt = $conn->query("SELECT * FROM payment ORDER BY ID DESC LIMIT 1");
$stmt->execute();
...
foreach($rows as $row){
    ...
    if($res['Body']['stkCallback']['ResultCode'] == '1032'){
        $sql = $conn->query("UPDATE `payment` SET `Status` = 'CANCELLED' WHERE `orders`.`ID` = $ID");
        $rs = $sql->execute();
     }else{
        $sql = $conn->query("UPDATE `payment` SET `Status` = 'SUCCESS' WHERE `orders`.`ID` = $ID");
        $rs = $sql->execute();
     }
